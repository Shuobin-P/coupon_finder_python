...
def create_app(test_config=None):
        ...
        coupon_finder_engine = create_engine("mysql://"+config['mysql']['username']+":"+str(config['mysql']['password'])+"@"+str(config['mysql']['winServer'])+"/coupon_finder?charset=utf8",pool_size=10, max_overflow=20)
...
def get_mq_connection():
    if 'mq_connection' not in g:
        ...
        g.mq_connection = pika.BlockingConnection(pika.ConnectionParameters(config['rabbitmq']['winServer'],credentials=credentials))
